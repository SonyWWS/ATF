...
namespace UnitTests.Atf
{
    ...
    public class TestPolygon3F
    {
        ...
        private void TestToStringResults(Polygon3F o, string s, string listSeparator, string decimalSeparator)
        {
            string[] results = s.Split(new[] { listSeparator,"(",")" }, StringSplitOptions.RemoveEmptyEntries);
            Assert.AreEqual(results.Length, 1 + 9);
            Assert.True(results[1].Contains(decimalSeparator));//spot check that floats have correct decimal points
            Assert.AreEqual(float.Parse(results[1]), o.Vertices[0].X);
            Assert.AreEqual(float.Parse(results[2]), o.Vertices[0].Y);
            Assert.AreEqual(float.Parse(results[3]), o.Vertices[0].Z);
        }
    }
}
